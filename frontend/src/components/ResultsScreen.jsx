  ...
  return (
    <div className="max-w-4xl mx-auto text-center">
      <div className="bg-gray-800 rounded-lg p-8 mb-8">
        ...
        <p className="text-xl text-gray-300 mb-8">{performance.message}</p>

        {/* Final Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div>
            <div className="text-4xl font-bold text-redhat mb-2">{session.score}</div>
            <div className="text-sm text-gray-400">Final Score</div>
          </div>
          <div>
            <div className="text-4xl font-bold text-green-500 mb-2">{session.correct_guesses}</div>
            <div className="text-sm text-gray-400">Correct</div>
          </div>
          <div>
            <div className="text-4xl font-bold text-blue-500 mb-2">{session.total_guesses}</div>
        ...
        {!submitted ? (
          <form onSubmit={handleSubmit} className="max-w-md mx-auto">
            <h3 className="text-xl font-semibold mb-4">Submit to Leaderboard</h3>
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg focus:outline-none focus:border-redhat"
                maxLength={30}
              />
              <button
                type="submit"
                disabled={submitting}
                className="px-6 py-3 bg-redhat hover:bg-redhat-dark disabled:bg-gray-600 rounded-lg font-semibold transition"
              >
          ...
          <div className="bg-green-900 bg-opacity-30 p-4 rounded-lg mb-4">
            <p className="text-green-400 font-semibold">Score submitted to leaderboard!</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-800 rounded-lg p-8 mb-8">
        ...
        <div className="text-left space-y-4 text-gray-300">
          <p>
            <strong className="text-white">Synthetic data generation</strong> uses AI models to create realistic fake data that mimics real examples. This technology is valuable for:
          </p>
      ...
      <button
        onClick={onRestart}
        className="px-8 py-4 bg-redhat hover:bg-redhat-dark rounded-lg font-semibold text-lg transition transform hover:scale-105"
      >
        Play Again
      </button>
    </div>
